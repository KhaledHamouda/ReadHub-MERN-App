...
export default function AdminBooks() {
  ...
  const handleDelete = (id: string) => {
    ...
    if (agree) {
      axios
        .delete(`http://localhost:3100/books/${id}`)
        .then((res) => {
          if (res.data) {
            handleDisplay();
          } else {
  ...
  const handleUpdateSubmit = (id: string, e: React.FormEvent) => {
    if (currentBook) {
      ...
      axios
        .put(`http://localhost:3100/books/${id}`, currentBook)
        .then((res) => {
          if (res) {
            handleDisplay();
          } else {
          ...
          <div className="collapse navbar-collapse" id="navbarText">
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item mx-2">
                <a
                  className="nav-link fw-bold rounded border border-primary bg-warning"
                  href="/admin/category"
                >
                  Categories
                </a>
      ...
      {currentBook && showForm && (
        ...
        >
          ...
          <button
            type="button"
            onClick={(e) => {
              handleUpdateSubmit(currentBook._id, e);
            }}
