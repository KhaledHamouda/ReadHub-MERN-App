...
route.get("/:id", async (req, res) => {
  try {
    ...
    const targetBook = await Book.findById(book_id)
      .populate("authorId", "authorFirstName authorLastName") // Populate author fields
      .populate("categoryId", "categoryName"); // Populate category fields
...
route.post("/", (req, res) => {
  ...
  // fetch ids from DB
  Category.where("categoryName")
    .equals(categoryName)
    .select("_id categoryId")
    .then((cat) => {
      // Ensure that `catId` is resolved before proceeding
      return Author.where("authorFirstName")
        .equals(authFirstName)
        .where("authorLastName")
        .equals(authSecondName)
        .select("_id authorId")
        .then((auth) => {
